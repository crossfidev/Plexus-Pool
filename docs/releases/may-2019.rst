...
performed at the launch of new mineplex node.
For this reason, once the storage migrated, it will not be possible to
use the old mineplex node.
...
storage directory `~/.mineplex-node`, begin by gracefully stopping the
node.
Back up your storage and upgrade it, it will only take a few seconds::

  cp -r ~/.mineplex-node mineplex-node-$(date +%Y%m%d)
...
We can proceed to export a snapshot::
  ...
  mineplex-node snapshot export mainnet-$(date +%Y%m%d).full

The default is to snapshot the chain at the position of our last
checkpoint, which is usually 5 cycles in the past.
This is the safest option as the mineplex node guarantees that no
...
Alternatively we can also specify the block to snapshot::
  ...
  mineplex-node snapshot export mainnet-$HASH.full --block $HASH
...
(without forgetting, if you need, to save ``{identity,peers,config}.json``)::
    ...
    rm -r ~/.mineplex-node
    mineplex-node snapshot import ./mainnet-*.full

There should be a rather smaller `~/.mineplex-node` directory populated
...
- Commits:
    ...
    - ``333782792``: ``Alpha/Daemons: use the mineplex-client context to select the chain``
...
- Fixes
- Commits:
    - ``d7ba8bb07``: ``Resto: Fix #480: Keep the host: set by the user with mineplex_SIGNER_HTTP_HEADERS``
...
- Commits:
    ...
    - ``39d1f229c``: ``Only publish docker/docs on mineplex/mineplex``
    ...
    - ``30816ed09``: ``Build,CI: Add the mineplex-event-logging library``
    ...
    - ``e5c396997``: ``[bin client]: remove wrong dependency to mineplex-shell``
    - ``af5ed9cd9``: ``Opam: update dependencies``
    - ``1491675ea``: ``Account for ocaml.4.07.1 changes in mineplex``
    ...
    - ``d6a3b46f1``: ``Opam packaging: install mineplex_PROTOCOL as part of mineplex-embedded-protocol-%.raw``
    - ``4511710d4``: ``Flextesa: voting command takes the mineplex_PROTOCOL file as argument``
    - ``b6619de3a``: ``lib_network_sandbox: Make it 32bits arch compatible``
    - ``6e91d5ec2``: ``Opam packaging: fix mineplex-client-alpha-commands deps``
