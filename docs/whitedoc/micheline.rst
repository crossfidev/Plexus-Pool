...
conversion is useful for example to interact with a mineplex node using
...
``mineplex-codec describe alpha.script binary schema``.

Usage of the OCaml Micheline library
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In the mineplex codebase, Micheline nodes are handled by a small library
named ``lib_micheline``. This library is used in the mineplex client
(responsible for parsing the Michelson files, expanding macros, and
encoding the result to JSON) and in the mineplex protocol (responsible
...
The following links are not part of the mineplex OCaml code base but are
...
- An online converter between Micheline files and their JSON encoding
  ...
  https://smart-contracts-micheline-michelson-translator-for-mineplex.scalac.io
  developed as part of Cryptonomic hackathon and documented here:
  https://scalac.io/we-hakked-mineplex-and-created-a-micheline-michelson-translator/
