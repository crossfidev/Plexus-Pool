...
current_hash_genesis=`jq '.hash' < src/proto_genesis/lib_protocol/src/mineplex_PROTOCOL | tr -d '"'`
...
grep -v '"hash"' < src/proto_genesis/lib_protocol/src/mineplex_PROTOCOL > $genesis_tmpdir/src/mineplex_PROTOCOL
new_hash_genesis=`./mineplex-protocol-compiler -hash-only $genesis_tmpdir/tmp $genesis_tmpdir/src`
...
current_hash_alpha=`jq '.hash' < src/proto_alpha/lib_protocol/src/mineplex_PROTOCOL | tr -d '"'`
...
grep -v '"hash"' < src/proto_alpha/lib_protocol/src/mineplex_PROTOCOL > $alpha_tmpdir/src/mineplex_PROTOCOL
new_hash_alpha=`./mineplex-protocol-compiler -hash-only $alpha_tmpdir/tmp $alpha_tmpdir/src`
