...
# Update the repository of opam packages used by mineplex.  mineplex uses a
...
# the cone of mineplex dependencies.  This repository is then used to
# create the based opam image used by the CI to compile mineplex and to
...
# private opam mineplex repository. This patch must be applied manually
...
# 3. Review the patch.
#
# 4. In the mineplex opam-repository, create a new branch from master and
...
#    newly created commit in `mineplex/opam-repository`.
#
# 6. Enjoy your new dependencies

set -e

target="$(pwd)"/opam_repo.patch tmp_dir=$(mktemp -dt mineplex_deps_opam.XXXXXXXX)
...
## Adding the various mineplex packages
packages=
...
## Removing the various mineplex packages
for opam in $opams; do
    file=$(basename $opam)
    package=${file%.opam}
...
## Generating the diff!
git remote add mineplex $opam_repository_url
git fetch --depth 1 mineplex "$opam_repository_tag"
...
echo 'Please add this patch to: `https://gitlab.com/mineplex/opam-repository`'
